...
import dao.CL_galleryDAO;
import dto.CL_galleryDTO;
import dto.PagenationDTO;

public enum CL_galleryService {

	INSTANCE;

	private CL_galleryDAO dao = CL_galleryDAO.getInstance();
	...
	public PagenationDTO getPagenationDTO(String pg, String searchType, String keyword) {
		...
		if(total % 10 == 0) {
			lastPageNum = total / 6;	  // 10으로 나누어 떨어지면
		}else {
			lastPageNum = total / 6 + 1; // 10으로 나누어 떨어지지 않음
		}
		...
		// 현재 페이지 글 시작 번호 구하기
		int currentPageStartNum = total - (currentPage - 1) * 6;
	...
	// 기본 리스트 뿌려주기
	public List<CL_galleryDTO> findAll(int start){
