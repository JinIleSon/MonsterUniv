...
import dao.Adm_counselDAO;
import dto.Adm_counselDTO;
import dto.PagenationDTO;

public enum Adm_counselService {

	INSTANCE;

	private Adm_counselDAO dao = Adm_counselDAO.getInstance();
	...
	// 기본 리스트 뿌려주기
	public List<Adm_counselDTO> findAll(int start){
		return dao.selectAll(start);
	}
	// 검색한 결과
	public List<Adm_counselDTO> findAllSearch(int start, String searchType, String keyword){
		return dao.selectArticleSearch(start, searchType, keyword);
	}
	// main에서 불러옴
	public List<Adm_counselDTO> findAllThree(){
