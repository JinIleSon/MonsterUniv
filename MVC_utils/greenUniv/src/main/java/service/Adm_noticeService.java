...
import dao.Adm_noticeDAO;
import dto.Adm_noticeDTO;
import dto.PagenationDTO;

public enum Adm_noticeService {

	INSTANCE;

	private Adm_noticeDAO dao = Adm_noticeDAO.getInstance();
	...
	// 기본 리스트 뿌려주기
	public List<Adm_noticeDTO> findAll(int start){
		return dao.selectAll(start);
	}
	// 검색한 결과
	public List<Adm_noticeDTO> findAllSearch(int start, String searchType, String keyword){
		return dao.selectArticleSearch(start, searchType, keyword);
	}
	// main에서 불러옴
	public List<Adm_noticeDTO> findAllThree(){
