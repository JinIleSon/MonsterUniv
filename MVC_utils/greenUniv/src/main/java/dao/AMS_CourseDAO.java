...
public class AMS_CourseDAO extends DBHelper {
	...
	public int selectCountTotal() {
		...
		try {
			...
			rs = stmt.executeQuery(Sql_AMS_subpages.SELECT_COURSE_COUNT_TOTAL);
	...
	public List<AMS_courseDTO> selectAll(int start) {
		...
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(Sql_AMS_subpages.SELECT_COURSE);
			psmt.setInt(1, start);
	...
	public int selectCountSearch(String searchType, String keyword) {
		int total = 0;
		StringBuilder sql = new StringBuilder(Sql_AMS_subpages.SELECT_COURSECOUNT_SEARCH);
	...
	public List<AMS_courseDTO> selectCourseSearch(int start, String searchType, String keyword) {
		...
		StringBuilder sql = new StringBuilder(Sql_AMS_subpages.SELECT_COURSE_SEARCH);
		
		if(searchType.equals("lname")) {
			sql.append(Sql_AMS_subpages.COURSE_SEARCH_WHERE_LNAME);
		} else if(searchType.equals("prof")) {
			sql.append(Sql_AMS_subpages.COURSE_SEARCH_WHERE_PROF);
		}
		
		sql.append(Sql_AMS_subpages.COURSE_SEARCH_ORDER_DEPTCODE); 
