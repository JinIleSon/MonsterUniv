...
import util.Sql_comm_emp;

public class Commu_empDAO extends DBHelper{

	private final static Commu_empDAO INSTANCE = new Commu_empDAO();
	public static Commu_empDAO getInstance() {
		return INSTANCE;
	}
	...
	public List<Commu_empDTO> selectAll(int start){
		...
		try {
			...
			while (rs.next()) {
				...
				dto.setCondition(rs.getString(2));
	...
	public int selectCountSearch(String searchType, String keyword) {
		...
		if(searchType.equals("title")) {
			...
			sql.append(Sql_comm_emp.SEARCH_WHERE_ALL);
			allOrNot = true;
