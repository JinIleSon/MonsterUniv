...
public class SearchListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 수정 필요 : 로그인 구현과 연동
		// cCode(G01940)는 student의 학과명과 collmaj의 학과명을 비교하여 collmaj 테이블에서 들고옴.
		PagenationDTO pagenationDTO = regService.getPagenationInfo("G01940", pg, searchType, keyword);
		
		int start = pagenationDTO.getStart();
		logger.debug("start : "+start);
		List<SA_regDTO> dtoList = regService.findBySearch("G01940", start, searchType, keyword);
